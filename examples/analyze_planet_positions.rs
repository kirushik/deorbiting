...
fn main() {
    ...
    println!(
        "   Earth at: ({:.4}, {:.4}) AU, angle {:.1}°",
        earth_pos.x / AU_TO_METERS,
        earth_pos.y / AU_TO_METERS,
    ...
    let asteroid_pos = DVec2::new(
        earth_r * asteroid_angle.cos(),
        earth_r * asteroid_angle.sin(),
    );
    ...
    println!(
        "   Asteroid at: ({:.4}, {:.4}) AU, angle {:.1}°",
        asteroid_pos.x / AU_TO_METERS,
        asteroid_pos.y / AU_TO_METERS,
        asteroid_angle.to_degrees()
    );
    println!(
        "   Velocity: ({:.2}, {:.2}) km/s = {:.2} km/s retrograde",
        vel.x / 1000.0,
        vel.y / 1000.0,
        vel.length() / 1000.0
    );
    ...
    println!(
        "   Jupiter at J2000: ({:.4}, {:.4}) AU, angle {:.1}°",
    ...
    let asteroid_pos = DVec2::new(
        approach_r * approach_angle.cos(),
        approach_r * approach_angle.sin(),
    );
    ...
    println!(
        "   Suggested asteroid position: ({:.4}, {:.4}) AU",
        asteroid_pos.x / AU_TO_METERS,
        asteroid_pos.y / AU_TO_METERS
    );
    println!(
        "   Suggested velocity: ({:.2}, {:.2}) km/s",
        vel.x / 1000.0,
        vel.y / 1000.0
    );
    ...
    println!(
        "   Suggested asteroid position: ({:.4}, {:.4}) AU",
        flyby_pos.x / AU_TO_METERS,
        flyby_pos.y / AU_TO_METERS
    );
    println!(
        "   Suggested velocity: ({:.2}, {:.2}) km/s",
        vel.x / 1000.0,
    ...
    let challenge_pos = DVec2::new(
        challenge_r * challenge_angle.cos(),
        challenge_r * challenge_angle.sin(),
    );
    ...
    println!(
        "   Asteroid at: ({:.4}, {:.4}) AU (90° ahead = ~91 days to collision)",
        challenge_pos.x / AU_TO_METERS,
        challenge_pos.y / AU_TO_METERS
    );
    println!(
        "   Velocity: ({:.2}, {:.2}) km/s retrograde",
        vel.x / 1000.0,
        vel.y / 1000.0
    );
...
fn simulate_jupiter_slingshot(data: &HashMap<&str, (f64, f64, f64, f64)>) {
    ...
    println!(
        "Initial: pos = ({:.2}, {:.2}) AU, speed = {:.2} km/s",
        pos.x / AU_TO_METERS,
        pos.y / AU_TO_METERS,
    ...
    while time < max_time {
        ...
        if day % 100 == 0 && day > 0 && ((time - dt) / DAY_SECONDS) as i32 != day {
            println!(
                "Day {:4}: pos ({:6.2}, {:6.2}) AU, speed {:.2} km/s, Jupiter dist {:.4} AU",
                day,
                pos.x / AU_TO_METERS,
                pos.y / AU_TO_METERS,
                vel.length() / 1000.0,
                jup_dist / AU_TO_METERS
            );
        }
    }
    ...
    println!(
        ...
        initial_speed / 1000.0,
        final_speed / 1000.0,
        (final_speed - initial_speed) / 1000.0
    );
}
