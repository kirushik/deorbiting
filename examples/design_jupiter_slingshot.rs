...
fn main() {
    ...
    println!("  Position: ({:.4}, {:.4}) AU", jupiter_pos_0.x / AU_TO_METERS, jupiter_pos_0.y / AU_TO_METERS);
...
fn test_trajectory(
    ...
    let vel = if prograde { tangent * v_circular * velocity_factor } else { -tangent * v_circular * velocity_factor };
    
    println!("Start: ({:.4}, {:.4}) AU, speed {:.2} km/s",
             pos.x / AU_TO_METERS, pos.y / AU_TO_METERS, vel.length() / 1000.0);
...
fn test_direct_intercept(planet_data: &HashMap<&str, (f64, f64, f64, f64)>) {
    ...
    println!("Start: ({:.4}, {:.4}) AU, speed {:.2} km/s",
             pos.x / AU_TO_METERS, pos.y / AU_TO_METERS, vel.length() / 1000.0);
    println!("Jupiter in 500 days: ({:.4}, {:.4}) AU",
             jupiter_future.x / AU_TO_METERS, jupiter_future.y / AU_TO_METERS);
...
fn test_aimed_approach(planet_data: &HashMap<&str, (f64, f64, f64, f64)>) {
    ...
    println!("Start: ({:.4}, {:.4}) AU, speed {:.2} km/s",
             pos.x / AU_TO_METERS, pos.y / AU_TO_METERS, vel.length() / 1000.0);
    println!("Jupiter now: ({:.4}, {:.4}) AU",
             jupiter_pos.x / AU_TO_METERS, jupiter_pos.y / AU_TO_METERS);
    
    simulate(pos, vel, planet_data, 500.0);
}

fn simulate(mut pos: DVec2, mut vel: DVec2, planet_data: &HashMap<&str, (f64, f64, f64, f64)>, max_days: f64) {
    ...
    while time < max_time {
        ...
        if jupiter_dist < 0.1 * AU_TO_METERS {
            ...
            println!("  Day {:4.0}: CLOSE APPROACH {:.6} AU ({:.0} Jupiter radii)",
                     day, jupiter_dist / AU_TO_METERS,
    ...
    println!("Results:");
    println!("  Closest Jupiter: {:.4} AU at day {:.0}", 
             closest_jupiter / AU_TO_METERS, closest_time / DAY_SECONDS);
    println!("  Velocity: {:.2} → {:.2} km/s (Δv = {:+.2} km/s)",
